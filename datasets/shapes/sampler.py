...
class ShapeSampler(nn.Module):
    @configurable
    def __init__(
        self, max_candidate=1, shape_prob=[], shape_candidate=[], is_train=True
    ):
        super().__init__()
    ...
    def from_config(cls, cfg, is_train=True, mode=None):
        ...
        candidate_classes = [
            getattr(sys.modules[__name__], class_name)(cfg, is_train)
    ...
    def forward(self, masks, boxes, max_candidate=50):
        ...
        if len(masks) == 0:
            ...
            return {
                ...
                "rand_shape": torch.stack([rand_masks]),
                "types": ["none"],
            }
        ...
        draw_funcs = random.choices(
            self.shape_candidate, weights=self.shape_prob, k=len(candidate_mask)
        )  # sample one shape, i.e., point
        rand_shapes = [
            d.draw(x, b).cuda()
            for d, x, b in zip(draw_funcs, candidate_mask, candidate_box)
        ]
        ...
        return {
            "gt_masks": candidate_mask,
            "rand_shape": torch.stack(rand_shapes),
            "types": types,
            "sampler": self,
        }
