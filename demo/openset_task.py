...
from DINOv.utils.visualizer import Visualizer
...
def task_openset(
    ...
    multi_scale_features2, mask_features2, _, _ = model.model.get_encoder_feature(
        batched_inputs_tgt
    )
    ...
    for image in in_context_examples:
        ...
        data["targets"][0]["pb"] = torch.tensor([1.0])  # FIXME 0 or 1
    ...
    # prompt to tgt image
    input_query_label_content_current = torch.stack(input_query_label_content_all).mean(
        0
    )
    masks, ious, ori_masks, scores_per_image_openset = (
        model.model.evaluate_demo_content_openset_multi_with_content_features(
            batched_inputs_tgt,
            mask_features2,
    ...
    for i, (pred_masks_pos, iou, _, _) in enumerate(
        zip(pred_masks_poses, ious, pred_masks_poses, pred_masks_poses)
    ):
