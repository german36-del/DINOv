...
from DINOv.dinov.utils import get_class_names
                ...
                for idx, video in enumerate(dataloader):
                    ...
                    for i in range(len(video[0])):
                        if "instances" in video[0][i]:
                            ...
                            break
                    rand_shape = (
                        video[0][cur_idx]["instances"].gt_masks.tensor[:, None] & False
                    )
                    ...
                    memory_content_label = [
                        PriorityQueue(maxsize=maxsize) for i in range(len(rand_shape))
                    ]
                    ious = None
                    for fid, frame2 in enumerate(video[0]):
                        if frame2["key_frame"].sum() > 0:
                            ...
                            rand_shape[frame2["key_frame"]] = frame2[
                                "instances"
                            ].gt_masks.tensor[:, None][
                                frame2["key_frame"]
                            ]  # ReTrack when the object disapper in some frames
                            rand_shape = rand_shape.squeeze(1)
                            print(
                                "fid, video[0].vid_name ",
                                fid,
                                video[0].vid_name,
                                frame2["key_frame"],
                            )
                        ...
                        multi_scale_features2, mask_features2, _, _ = (
                            model.model.get_encoder_feature(batched_inputs2)
                        )
                        if (
                            fid == 0 or ious[i] > -1.0
                        ):  # only put these with large confidence score
                            multi_scale_features, _, padded_h, padded_w = (
                                model.model.get_encoder_feature(batched_inputs)
                            )
                            (
                                input_query_label_content,
                                input_query_bbox_content,
                                attn_mask_content,
                            ) = model.model.get_visual_prompt_content_feature(
                                multi_scale_features, rand_shape, padded_h, padded_w
                            )
                            # focus on the most recent frame; record with a score
                            score = (
                                fid if fid else 10000
                            )  # always keep the reference first frame
                            if input_query_label_content.shape[1] > len(
                                memory_content_label
                            ):
                                input_query_label_content = (
                                    input_query_label_content.view(
                                        input_query_label_content.shape[0],
                                        len(memory_content_label),
                                        -1,
                                        input_query_label_content.shape[-1],
                                    )
                                )
                            for i, query in enumerate(
                                input_query_label_content.squeeze(0)
                            ):

                                if (
                                    memory_content_label[i].full()
                                    and len(memory_content_label[i].queue) == 1
                                ):
                                    continue
                                if memory_content_label[i].full():
                                    a = memory_content_label[i].get()

                                memory_content_label[i].put(
                                    (score, query.detach().clone())
                                )
                                # frame = frame2
                        ...
                        for instance_memory_content in memory_content_label:
                            instance_memory_content_current = torch.zeros_like(
                                instance_memory_content.queue[0][-1]
                            ).to(instance_memory_content.queue[0][-1])
                            for q in range(len(instance_memory_content.queue)):
                                # combine with memory
                                instance_memory_content_current = (
                                    instance_memory_content_current
                                    + instance_memory_content.queue[q][-1]
                                    .detach()
                                    .clone()
                                )
                            instance_memory_content_current = (
                                instance_memory_content_current
                                / len(instance_memory_content.queue)
                            )
                            input_query_label_content_current.append(
                                instance_memory_content_current
                            )
                        input_query_label_content_current = torch.stack(
                            input_query_label_content_current
                        )[None]
                        if len(input_query_label_content_current.shape) > 3:
                            input_query_label_content_current = (
                                input_query_label_content_current.flatten(1, 2)
                            )
                        masks, ious, ori_masks = (
                            model.model.evaluate_visual_prompt_refer_multi_with_content_features(
                                batched_inputs2,
                                mask_features2,
                        ...
                        if (
                            fid
                        ):  # use ground thruth first frame mask for the second frame
                        ...
                        if not save_score:
                            output_image.save(
                                os.path.join(
                                    output_folder, "{}.png".format(frame["frame_id"])
                                )
                            )
                        else:
                            output_image.save(
            ...
            with torch.no_grad():
                ...
                for idx, batch in enumerate(dataloader):
                    ...
                    with torch.autocast(device_type="cuda", dtype=torch.float16):
                        task = "get_content"
                        input_tokens_all, labels = model(
                            batch, inference_task=task, dataset_name=dataset_name
                        )
                    ...
                    for label, embedding in zip(labels, input_tokens_all):
                        ...
                        save_path = os.path.join(
                            save_cate_folder,
                            "id_{}_idx_{}.safetensors".format(
                                image_id, label_dict[label]
                            ),
                        )
        ...
        for dataloader, dataset_name in zip(dataloaders, dataset_names):
            ...
            with torch.no_grad():
                ...
                if "odinw" in dataset_name:
                    ...
                    names = get_class_names(
                        dataset_name, cfg["MODEL"].get("BACKGROUND", True)
                    )
                ...
                cat_dirs = [
                    int(cat)
                    for cat in cat_dirs
                    if os.path.isdir(os.path.join(output_dir, cat))
                ]
                cat_dirs.sort()
            ...
            if comm.is_main_process():
                if "seginw" in dataset_name or "odinw" in dataset_name:
                    if "seginw" in dataset_name:
                        score_mask_ap[dataset_name.split("_")[1]] = results["segm"][
                            "AP"
                        ]
                    ...
                    if "seginw" in dataset_name:
                        print(
                            "score_mask_ap ", sum(list(score_mask_ap.values())) / lent
                        )
...
def main(args=None):
    ...
    if comm.get_rank() == 0 and args.WANDB:
        ...
        init_wandb(
            cfg,
            cfg["OUTPUT_DIR"],
            entity=args.wandb_usr_name,
            job_name=cfg["OUTPUT_DIR"],
        )
