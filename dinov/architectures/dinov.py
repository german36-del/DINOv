...
import random
from DINOv.datasets.shapes.sampler import build_shape_sampler, ShapeSampler
import copy
...
from ..modules import (
    sem_seg_postprocess,
    HungarianMatcher,
    HungarianMatcherMany2Many,
    SetCriterionVisualOpenSet,
    SetCriterionReferOne,
    SetCriterionReferMany,
)
from ..language import build_language_encoder
from DINOv.utils.dist import get_world_size, all_gather
...
class DINOv(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.task_switch = {
            "coco": coco_on,
            "o365": o365_on,
            "pascal_part": pascal_part_on,
            "sam": sam_on,
            "coco_track": coco_track,
        }
        ...
        if coco_on:
            ...
            self.train_class_names[task] = get_class_names(
                "coco_2017_train_panoptic", background=background
            )
            self.train_class_names["ade"] = get_class_names(
                "ade20k_panoptic_train", background=background
            )
        ...
        if freeze_backbone_enc_decoder:
            for name, param in self.named_parameters():
                param.requires_grad = False
            print(
                "!!!!!!!!freeze_backbone_enc_decoder!!!!!!!!, except ", to_freeze_dict
            )
            for name, param in self.named_parameters():
                for f_name in to_freeze_dict:
    ...
    def from_config(cls, cfg):
        ...
        criterion_switch = {
            "open_set": criterion,
            "1o1_cm": criterion_1o1_content_match,
        ...
        shape_sampler = build_shape_sampler(
            cfg,
        )
        ...
        return {
            ...
            "openset_use_shape_sampler": dec_cfg.get(
                "OPENSET_USE_SHAPE_SAMPLER", False
            ),
            ...
            "freeze_backbone_enc_decoder": dec_cfg.get(
                "freeze_backbone_enc_decoder", False
            ),
        }
        ...
        if self.training:
            ...
            if self.task_switch["coco"]:
                ...
                data = (
                    ...
                    else batched_inputs["coco"]
                )
                losses_coco = self.forward_seg(
                    data, task=task, prediction_switch=prediction_switch
                )
                new_losses_coco = {}
            ...
            if self.task_switch["sam"]:
                ...
                losses_sam = self.forward_seg(
                    data, task="visual_refer", prediction_switch=prediction_switch
                )
                new_losses_sam = {}
            ...
            if "generic" in inference_task or "get_content" in inference_task:
                processed_results = self.forward_seg(
                    batched_inputs, task=inference_task, dataset_name=dataset_name
                )
            elif inference_task == "visual_openset":
                processed_results = self.evaluate_visual_openset(
                    batched_inputs, task=inference_task, dataset_name=dataset_name
                )
            else:
    ...
    def forward_seg(
        ...
        if self.training:
                ...
                if task == "visual_refer":
                    ...
                    if num_mask == 0:
                        print(
                            ...
                            gt_instances[0].gt_classes.shape[0],
                            "targets_per_image",
                            gt_instances[0],
                        )
                    if not coco_track:
                        ...
                        if self.max_num_instance > num_mask:
                            ...
                            rep = (
                                0
                                if num_mask == 0
                                else int(self.max_num_instance / num_mask) + 1
                            )
                    ...
                    targets["position"] = self.prepare_targets_interactive(
                        gt_instances,
                        images,
                        prediction_switch=prediction_switch,
                        index=index,
                    ...
                    targets["content"] = self.prepare_targets_visual_refer_seg(
                        gt_instances,
                        images,
                        prediction_switch=prediction_switch,
                        task="coco",
                    ...
                    targets["content"] = (
                        self.prepare_targets_visual_openset_batch_cross_gpu(
                            gt_instances_copy,
                            images,
                            task=task,
                            prediction_switch=prediction_switch,
                    ...
                    contain_fake = (
                        []
                    )  # indicator: if an image on a gpu contains no ground-truth instances
                    ...
                    if contain_fake_ or not self.cross_gpu_batch:
                        ...
                        print(
                            "fake new taragets ", torch.as_tensor(0.0).to("cuda").device
                        )
                        gt_instances_copy = copy.deepcopy(gt_instances)
                        targets["content"] = (
                            self.prepare_targets_visual_openset_batch_cross_gpu(
            ...
            input_tokens_all, labels = self.sem_seg_head(
                features, targets=targets, task=task, extra=prediction_switch
            )
            return input_tokens_all, labels

    def evaluate_visual_openset(
        self,
        batched_inputs,
        task="seg",
        prediction_switch={"part": True, "whole": True, "seg": True, "det": True},
        ...
        outputs, _ = self.sem_seg_head(
            features, targets=targets, task="visual_openset", extra=prediction_switch
        )
        ...
        if "det" not in task:
            ...
            self.semantic_on = self.panoptic_on = (
    ...
    def prepare_targets_interactive(
        self, targets, images, prediction_switch, task="seg", index=None
    ):
        """
        ...
        for targets_per_image in targets:
            ...
            if self.regenerate_point and box_start > 0:
                ...
                for i in range(box_start):
                    ...
                    center_point = (
                        True  # FIXME for evaluation sample the center as clicks
                    )
            ...
            if not self.training:
                ...
                auto_points = torch.tensor(
                    build_point_grid(self.point_per_side)
                ).cuda() * image_size_xyxy[:2].to(new_target["points"]).to(
                    torch.float32
                )

                new_target["points"] = (
                    box_ops.box_xyxy_to_cxcywh(
                        torch.cat([auto_points - 3, auto_points + 3], 1)
                    )
                    / image_size_xyxy
                )
                new_target["points"] = torch.tensor(
                    new_target["points"], dtype=torch.float32
                )
    ...
    def prepare_targets_visual_refer_seg(
        self, targets, images, prediction_switch, task="seg", index=None
    ):
        h_pad, w_pad = images.tensor.shape[-2:]
        new_targets = []
        for targets_per_image in targets:
            ...
            h, w = targets_per_image.image_size
            if not self.training:
                h_pad, w_pad = from_divisablity(
                    h, self.size_divisibility
                ), from_divisablity(w, self.size_divisibility)
            ...
            if len_gt == 0:
                ...
                new_target = {
                    "boxes": torch.ones(self.max_num_instance_content, 4)
                    .to(gt_masks)
                    .float(),
                    "rand_shape": torch.ones(
                        self.max_num_instance_content, h_pad, w_pad
                    )
                    .to(gt_masks)
                    .float(),
                    "points": torch.ones(self.max_num_instance_content, 4)
                    .to(gt_masks)
                    .float(),
                    "boxes_dn": torch.ones(self.max_num_instance_content, 4)
                    .to(gt_masks)
                    .float(),
                    "pb": torch.cat(
                        [
                            torch.zeros(self.max_num_instance_content - box_start),
                            torch.ones(box_start),
                        ],
                        0,
                    ),
                ...
                if prediction_switch["whole"]:
                    new_target["gt_whole_classes"] = (
                        torch.tensor([len(self.train_class_names[task]) - 1])
                        .repeat(self.max_num_instance_content)
                        .to(gt_masks.device)
                    )
                new_targets.append(new_target)
            ...
            box_start = self.max_num_instance_content
            point_coords = (
                torch.ones(self.max_num_instance_content, 4).to(padded_masks).float()
            )
            point_coords[:, :2] = 0.0
            ...
            if self.use_shape_sampler:
                ...
                content_dict = self.shape_sampler(
                    new_padded_masks, gt_boxes[index], self.max_num_instance_content
                )
    ...
    def prepare_targets_visual_openset_batch_cross_gpu(
        self, targets, images, task="seg", prediction_switch=None, cross_gpu=False
    ):
        """
        ...
        for targets_per_image in targets:
            ...
            if len_gt == 0:
                ...
                new_target = {
                    "boxes": torch.ones(max_num_instance_openset, 4)
                    .to(gt_masks)
                    .float(),
                    "rand_shape": torch.ones(max_num_instance_openset, h_pad, w_pad)
                    .to(gt_masks)
                    .float(),
                    "points": torch.ones(max_num_instance_openset, 4)
                    .to(gt_masks)
                    .float(),
                    "boxes_dn": torch.ones(max_num_instance_openset, 4)
                    .to(gt_masks)
                    .float(),
                    "pb": torch.cat(
                        [
                            torch.zeros(max_num_instance_openset - box_start),
                            torch.ones(box_start),
                        ],
                        0,
                    ),
                ...
                if prediction_switch["whole"]:
                    new_target["gt_whole_classes"] = new_target["labels"] = (
                        torch.tensor(0)
                        .repeat(max_num_instance_openset)
                        .to(gt_masks.device)
                    )
            ...
            box_start = max_num_instance_openset
            point_coords = (
                torch.ones(max_num_instance_openset, 4).to(padded_masks).float()
            )
            point_coords[:, :2] = 0.0
            if self.openset_use_shape_sampler:
                content_dict = self.shape_sampler(
                    padded_masks, gt_boxes, self.max_num_instance
                )
                rand_shape = content_dict["rand_shape"].cuda()
            else:
            ...
            new_target = {
                ...
                "ori_mask_num": len(targets_per_image.gt_classes),
                "labels": (
                    targets_per_image.gt_classes if prediction_switch["whole"] else None
                ),
                "masks": padded_masks,
                ...
                "gt_part_classes": (
                    targets_per_image.gt_part_classes
                    if targets_per_image.has("gt_part_classes")
                    and prediction_switch["part"]
                    else None
                ),
            ...
            for k, v in unique_category_examples_by_category.items():
                if k in batch_examples_by_category.keys():
                    batch_examples_by_category[k] = torch.cat(
                        [
                            batch_examples_by_category[k],
                            unique_category_examples_by_category[k],
                        ]
                    )
                else:
                    batch_examples_by_category[k] = (
        ...
        if not empty_flag and not cross_gpu:
            ...
            for i, (cat, examples) in enumerate(batch_examples_by_category.items()):
                    ...
                    max_example_num
                    if max_example_num < len(examples)
                    else len(examples)
                )
                if task == "get_content":
            ...
            if self.use_shape_sampler and task != "get_content":
                ...
                content_dict = self.shape_sampler(
                    all_rand_shape[sample_index],
                    all_rand_shape[sample_index],
                    max_num_instance_openset,
                )
                all_rand_shape[sample_index] = content_dict["rand_shape"].cuda()
            new_rand_shape_per_instance = [
                all_rand_shape[id_start_list[i] : id_start_list[i + 1]]
                for i in range(len(id_start_list) - 1)
            ]
            new_labels = torch.cat(new_labels)
            new_labels = new_labels[indices]
            new_labels_per_instance = [
                new_labels[id_start_list[i] : id_start_list[i + 1]]
                for i in range(len(id_start_list) - 1)
            ]

            for i, target in enumerate(new_targets):
                target["labels"] = target["gt_whole_classes"] = new_labels_per_instance[
                    i
                ]
    ...
    def filter_data_refer(self, src_boxes, mask_pred_results, src_ious, pred_ious):
        ...
        keep = src_ious > 0.5
        src_boxes, mask_pred_results, src_ious, pred_ious = keep_data(
            src_boxes, mask_pred_results, src_ious, pred_ious, keep
        )

        item_indice = nms(
            box_ops.box_cxcywh_to_xyxy(src_boxes), src_ious, self.nms_thersh
        )  # FIXME iou threshold
        ...
        mask_pred_results = torch.gather(
            mask_pred_results[None].repeat(level.shape[0], 1, 1, 1),
            1,
            level[:, :, None, None].repeat(
                1, 1, mask_pred_results.shape[-2], mask_pred_results.shape[-1]
            ),
        )

        return src_boxes, mask_pred_results, src_ious, pred_ious

    def filter_data_openset(
        self, src_boxes, mask_pred_results, src_ious, pred_ious, pred_score_openset
    ):
        ...
        while sum(keep) < 1:
            thresh = thresh - 0.04
            keep = scores_per_image_openset > thresh
        scores_per_image_openset, label_openset = (
            scores_per_image_openset[keep],
            label_openset[keep],
        )
    ...
    def get_visual_prompt_content_feature(
        self, multi_scale_features, rand_shape, padded_h, padded_w, vid_name="default"
    ):
        ...
        new_rand_shape = torch.zeros(num_instance, padded_h, padded_w).to(
            targets[0]["rand_shape"]
        )
        ...
        input_query_label_content, input_query_bbox_content, attn_mask_content = (
            self.sem_seg_head.predictor.forward_get_content_feature(
                multi_scale_features, None, targets=targets
            )
        )

        return input_query_label_content, input_query_bbox_content, attn_mask_content

    def evaluate_visual_prompt_refer_multi_with_content_features(
        self,
        batched_inputs,
        mask_features,
        multi_scale_features,
        input_query_label_content,
        input_query_bbox_content,
        attn_mask_content,
        padded_h,
        padded_w,
        level=[0, 1, 2, 3, 4, 5],
        return_src_ious=False,
    ):
        ...
        auto_points = (
            torch.tensor(build_point_grid(self.point_per_side)).cuda()
            * image_size_xyxy[:2]
        )

        boxes_dn = (
            box_ops.box_xyxy_to_cxcywh(torch.cat([auto_points - 3, auto_points + 3], 1))
            / image_size_xyxy
        )
        ...
        targets = targets_p
        outputs, mask_dict = (
            self.sem_seg_head.predictor.forward_refer_image_with_extracted_content(
                multi_scale_features,
                mask_features,
                None,
                input_query_label_content,
        ...
        mask_pred_results = torch.index_select(
            mask_pred_results.view(
                src_ious.shape[0],
                src_ious.shape[1],
                mask_pred_results.shape[1],
                mask_pred_results.shape[2],
            ),
            1,
            level,
        ).flatten(0, 1)
        src_boxes = torch.index_select(
            src_boxes.view(src_ious.shape[0], src_ious.shape[1], src_boxes.shape[-1]),
            1,
            level,
        ).flatten(0, 1)
        pred_ious = torch.index_select(
            pred_ious.view(pred_ious.shape[0], src_ious.shape[0], src_ious.shape[1]),
            -1,
            level,
        ).flatten(-2, -1)
        src_ious = torch.index_select(src_ious, -1, level)
        src_ious = src_ious.flatten(0, 1)
        src_boxes, mask_pred_results, src_ious, pred_ious = self.filter_data_refer(
            src_boxes, mask_pred_results, src_ious, pred_ious
        )
        ...
        ori_masks = pred_masks[:, : image_size[0], : image_size[1]].expand(
            1, -1, -1, -1
        )[0]
        if self.sem_seg_postprocess_before_inference:
    ...
    def evaluate_demo_content_openset_multi_with_content_features(
        self,
        batched_inputs,
        mask_features,
        multi_scale_features,
        ...
        targets = None
        outputs, mask_dict = (
            self.sem_seg_head.predictor.forward_openset_image_with_extracted_content(
                multi_scale_features,
                mask_features,
                None,
                input_query_label_content,
        ...
        src_boxes, mask_pred_results, src_ious, pred_ious, scores_per_image_openset = (
            self.filter_data_openset(
                src_boxes, mask_pred_results, src_ious, pred_ious, pred_score_openset
            )
        )
    ...
    def instance_inference(self, mask_cls, mask_pred, mask_box_result):
        ...
        if self.sem_seg_postprocess_before_inference:
            mask_scores_per_image = (
                mask_pred.float().sigmoid().flatten(1)
                * result.pred_masks.float().flatten(1)
            ).sum(1) / (result.pred_masks.float().flatten(1).sum(1) + 1e-6)
        else:
